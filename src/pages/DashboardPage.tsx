  ...
  return (
    <div className="page-shell">
      <section className="glass-card overflow-hidden p-5 sm:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-cocoa-800/55">Live selling</p>
        <div className="mt-4 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-cocoa-800/70">
              Built mobile-first so the operator can sell from the session screen with large controls and minimal typing.
            </p>
          </div>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <article className="glass-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-lg font-bold text-cocoa-900">Session snapshot</h3>
              <p className="mt-1 text-sm text-cocoa-800/65">
                Quick summary for the current selling window.
              </p>
            </div>
            {loadingSession ? <span className="text-xs font-bold text-cocoa-800/55">Syncing...</span> : null}
          </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-3xl border border-cocoa-900/10 bg-white/70 p-4">
                  ...
                  <p className="mt-1 font-bold text-mint-500">{activeSession.status}</p>
                </div>
                <div className="rounded-3xl border border-cocoa-900/10 bg-white/70 p-4">
                  <p className="text-sm text-cocoa-800/60">Live total</p>
                  <p className="mt-1 font-bold text-cocoa-900">{formatCurrency(totals.estimatedTotal)}</p>
                </div>
              </div>
            </div>
          ) : (
        ...
        <article className="glass-card p-5">
          ...
          <div className="mt-4 grid gap-3">
            <Link to="/session">
              <Button fullWidth>{activeSession ? 'Continue selling' : 'Start selling'}</Button>
            </Link>
            <Link to="/candies">
